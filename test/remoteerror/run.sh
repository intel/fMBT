...
# - crash in remote aal init
# - crash in remote aal guard
# - crash in remote aal body
# - crash in remote aal adapter
# - crash in remote aal observe
# - crash in remote adapter init
# - crash in remote adapter execute
...
for WHEN in "load" "init" "iguard" "iadapter" "ibody" "oguard" "oadapter" "obody"; do
    for WHAT in "raise" "crash" "print"; do
        for HEURISTIC in "random" "lookahead(2)"; do
            echo "AAL, heur=$HEURISTIC, BUG=$WHEN-$WHAT" >> $LOGFILE
...
EOF
            ...
            if [ "$FMBTSTATUS" != "84" ]; then
                echo "AAL, heur=$HEURISTIC, BUG=$WHEN-$WHAT, exit status $FMBTSTATUS, expected 84" >>$LOGFILE
                failure_count=$(( $failure_count + 1 ))
            fi
            
            if [ "$WHAT" == "crash" ] && ! grep -q 'Segmentation' fmbt-output.txt; then
                echo "Segmentation fault missing in fmbt-output.txt" >>$LOGFILE
                failure_count=$(( $failure_count + 1 ))                
            elif [ "$WHAT" == "raise" ] && ! grep -q 'BogusException' fmbt-output.txt; then
                echo "Raised exception missing in fmbt-output.txt" >>$LOGFILE
                failure_count=$(( $failure_count + 1 ))                
            elif [ "$WHAT" == "print" ] && ! grep -q 'rubbishFromAAL' fmbt-output.txt; then
                echo "Rubbish printed from AAL is missing in fmbt-output.txt" >>$LOGFILE
                failure_count=$(( $failure_count + 1 ))                
            fi
        done
    done
