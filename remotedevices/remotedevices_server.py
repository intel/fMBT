...
class Devices(object):
    ...
    def _wolock_match(self, **matchArgs):
        matchingSerials = set([
            ...
            (not "free" in matchArgs or (matchArgs["free"] == str(self.available(self._infos[i])))) and
            (not "busy" in matchArgs or (matchArgs["busy"] == str(not self.available(self._infos[i]))))])
