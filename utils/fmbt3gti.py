...
def _convert(srcFile, convertArgs, dstFile):
    if isinstance(convertArgs, (str, bytes)):
...
def _boxOnRegion(x1_y1_x2_y2, minX_minY_maxX_maxY, partial=True):
    x1, y1, x2, y2 = x1_y1_x2_y2
    minX, minY, maxX, maxY = minX_minY_maxX_maxY
    if partial:
...
def _edgeDistanceInDirection(xy_coords, wh, direction):
    width, height = wh
    x, y = _intCoords(xy_coords, wh)
...
class OirEngine(OrEngine):
    ...
    def findBitmap(self, screenshot, bitmap, **kwargs):
        ...
        if os.access(bitmapLocsFilename, os.R_OK):
            ...
            try:
                bboxList = eval(open(bitmapLocsFilename).read().strip())
                foundItems = []
...
class _OirRc(object):
    ...
    def __init__(self, directory):
        ...
        for line in open(oirRcFilepath):
            line = line.strip()
...
class GUITestInterface(object):
    ...
    def drag(self, x1y1, x2y2, delayBetweenMoves=0.01,
        ...
        """
        x1, y1 = self.intCoords(x1y1)
        x2, y2 = self.intCoords(x2y2)
        ...
        for i in list(range(0, movePoints)):
    ...
    def enableVisualLog(self, filenameOrObj,
        ...
        if type(filenameOrObj) == str:
            try:
                outFileObj = open(filenameOrObj, "w", encoding='utf-8')
    ...
    def intCoords(self, xy_coords):
        """
        Convert floating point coordinate values in range [0.0, 1.0] to
        screen coordinates.
        """
        size_tup = self.screenSize()
        width, height = size_tup[0], size_tup[1]
        return _intCoords(xy_coords, (width, height))

    def relCoords(self, xy_coords):
        """
        Convert coordinates (x, y) to relative coordinates in range [0.0, 1.0].
        """
        width, height = self.screenSize()
        ix, iy = _intCoords(xy_coords, (width, height))
    ...
    def _archiveScreenshots(self):
        ...
        freeScreenshots = [filename
                           for (filename, refCount) in self._screenshotRefCount.items()
                           if refCount == 0]
    ...
    def swipe(self, xy_coords, direction, distance=1.0, **dragArgs):
        """
    ...
    def tap(self, xy_coords, long=_USE_DEFAULTS, hold=_USE_DEFAULTS,
...
class Screenshot(object):
    ...
    def findItemsByBitmap(self, bitmap, **oirFindArgs):
        print("bitmap is: {0}".format(bitmap))
                    ...
                    if (x, y-1) in coord2item:
                        ...
                        if itemA:
                            ...
                            if itemB != itemA:
                                ...
                                for ax in list(range(itemA._bbox[0], itemA._bbox[2]+1)):
                                    for ay in list(range(itemA._bbox[1], itemA._bbox[3]+1)):
...
class _VisualLog:
    def __init__(self, device, outFileObj,
        ...
        for a in attrs + winattrs:
            if hasattr(device, a):
                m = getattr(device, a)
                setattr(device, m.__name__, self.genericLogger(m))
    ...
    def close(self):
        if self._outFileObj != None:
            ...
            if self._bytesToFile > 0:
                for c in list(range(len(self._callStack))):
    ...
    def open(self, newFormattedFilename):
        ...
        self._outFileObj = open(self._formattedOutFilename, "w")
    ...
    def logCall(self, img=None, width="", imgTip=""):
        ...
        for key, value in argv.locals['kwargs'].items():
   ...
   $( document ).ready(function() {
    ...
    def genericLogger(loggerSelf, origMethod):
        def origMethodWRAP(*args, **kwargs):
            ...
            loggerSelf.logReturn(retval, tip=origMethod.__name__)
            return retval
        loggerSelf.changeCodeName(origMethodWRAP, origMethod.__name__ + "WRAP")
    ...
    def dragLogger(loggerSelf, origMethod):
        def dragWRAP(*args, **kwargs):
            ...
            try:
                ...
                loggerSelf.logReturn(retval, img=highlightFilename, width=loggerSelf._screenshotWidth, tip=origMethod.__name__)
            except:
                loggerSelf.logReturn(str(retval) + " (no screenshot available)", tip=origMethod.__name__)
            return retval
        return dragWRAP
    ...
    def refreshScreenshotLogger(loggerSelf, origMethod):
        def refreshScreenshotWRAP(*args, **kwargs):
            ...
            if retval != None:
                ...
                loggerSelf.logReturn(retval, img=retval, tip=origMethod.__name__)
    ...
    def tapLogger(loggerSelf, origMethod):
        def tapWRAP(*args, **kwargs):
            ...
            try:
                ...
                loggerSelf.logReturn(retval, img=highlightFilename, width=loggerSelf._screenshotWidth, tip=origMethod.__name__, imgTip=loggerSelf._device.screenshot()._logCallReturnValue)
            except:
                loggerSelf.logReturn(str(retval) + " (no screenshot available)", tip=origMethod.__name__)
            return retval
        return tapWRAP
    ...
    def findItemsByBitmapLogger(loggerSelf, origMethod, screenshotObj):
        def findItemsByBitmapWRAP(*args, **kwargs):
            ...
            retval = loggerSelf.doCallLogException(origMethod, args, kwargs)
            if len(retval) == 0:
                loggerSelf.logReturn("not found in", img=screenshotObj, tip=origMethod.__name__)
            else:
                foundItems = retval
                ...
                loggerSelf.logReturn([str(quiItem) for quiItem in retval], img=highlightFilename, width=loggerSelf._screenshotWidth, tip=origMethod.__name__, imgTip=screenshotObj._logCallReturnValue)
            return retval
        ...
        def findItemsByOcrWRAP(*args, **kwargs):
            ...
            if len(retval) == 0:
                loggerSelf.logReturn("not found in words " + str(screenshotObj.dumpOcrWords()),
                                     img=screenshotObj, tip=origMethod.__name__)
            else:
                foundItem = retval[0]
                screenshotFilename = screenshotObj.filename()
                highlightFilename = loggerSelf.highlightFilename(screenshotFilename)
                ...
                loggerSelf.logReturn([str(retval[0])], img=highlightFilename, width=loggerSelf._screenshotWidth, tip=origMethod.__name__, imgTip=screenshotObj._logCallReturnValue)
            return retval
    ...
    def changeCodeName(self, func, newName):
        ...
        func.__code__ = types.CodeType(
            c.co_argcount, 0, c.co_nlocals, c.co_stacksize, c.co_flags,
