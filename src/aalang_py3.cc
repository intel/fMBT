...
/* Common helpers to Python 2 & 3, see aalang_py.cc */
...
/* End of common helpers to Python 2 & 3, see aalang_py.cc */

std::string python3_lineno_wrapper(const std::string& filename,int lineno,
                                  const std::string& funcname,int trim,int ind,
                                  const std::string& human=")")
{
  if (lineno) {
    return indent(ind,funcname + ".__code__ = aalmodel.setCodeFileLine(" +
                  funcname + ".__code__, '''" + filename + "''', " +
                  to_string(lineno-trim) + human ) + "\n";
  }
  return "";
}

std::string python3_lineno_wrapper(const codefileline& cfl,
                                  const std::string& funcname,int trim,int ind,
                                  const std::string& human=")")
{
  return python3_lineno_wrapper(cfl.second.first,cfl.second.second,funcname,
                               trim,ind,human);
}



std::string aalang_py3::action_helper(const codefileline& cfl,std::string s,
...
const std::string aalang_py3::class_name() const
{
  return "_gen_" + *name;
}

const std::string aalang_py3::serialN(const std::string postfix, bool cls) const
{
...
const std::string aalang_py3::serialN_1(const std::string postfix, bool cls) const
{
...
const int aalang_py3::serial_stackN_1() const
{
  std::list<int>::const_iterator outer_serial = serial_stack.end();
  --outer_serial; --outer_serial;
  return *outer_serial;
...
void aalang_py3::set_starter(std::string* st,const char* file,int line,int col)
{
  s+=indent(0, *st)+"\n";
  delete st;
}
...
void aalang_py3::set_name(std::string* name,bool first,ANAMETYPE t)
{
  if (first) {
...
aalang_py3::~aalang_py3()
{
  if (name)
    delete name;
}

void aalang_py3::set_namestr(std::string* _name)
{
  name=_name;
  s+="import aalmodel\n"
...
void aalang_py3::set_variables(std::string* var,const char* file,int line,int col)
{
...
void aalang_py3::set_istate(std::string* ist,const char* file,int line,int col)
{
  model_init_counter++;
  ...
  s += "\n    def " + funcname + "():\n" + variables +
    indent(8, *ist) + "\n" +
    indent(8, "pass") + "\n";
  s += python3_lineno_wrapper(file,line,funcname,1+m_lines_in_vars,4);
  s += indent(4,"initial_state_list.append("+funcname+")")+"\n";
  s += indent(4,"push_variables_set.update("+funcname+".__code__.co_names)")+"\n";
  delete ist;
}

void aalang_py3::set_ainit(std::string* iai,const char* file,int line,int col)
{
...
void aalang_py3::set_aexit(std::string* iai,const char* file,int line,int col)
{
...
void aalang_py3::set_tagname(std::string* name,bool first)
{
  if (first) {
    ma_save.push(m_guard);
    ma_save.push(m_body);
...
void aalang_py3::next_tag()
{
  ...
  for (unsigned int i = 0; i < multiname.size(); i++) {
    ...
    s+=python3_lineno_wrapper(m_guard,funcname,2+m_lines_in_vars,4,
    ...
    if (adapter) {
      ...
      s+=python3_lineno_wrapper(m_adapter,funcname,2+m_lines_in_vars,4,
...
void aalang_py3::set_guard(std::string* gua,const char* file,int line,int col)
{
  std::string tmp=*gua;
...
void aalang_py3::parallel(bool start,std::list<std::string>* __params) {
  bool single=false;
...
void aalang_py3::serial(bool start,std::list<std::string>* __params) {
...
void aalang_py3::set_push(std::string* p,const char* file,int line,int col)
{
  push=*p;
  delete p;
}

void aalang_py3::set_pop(std::string* p,const char* file,int line,int col)
{
  pop=*p;
  delete p;
}

void aalang_py3::set_body(std::string* bod,const char* file,int line,int col)
{
  std::string tmp=*bod;
  if (bod!=&default_body)
    delete bod;
...
void aalang_py3::set_adapter(std::string* ada,const char* file,int line,int col)
{
  adapter = true;
  std::string tmp=*ada;
...
void aalang_py3::next_action()
{
  ...
  for (unsigned int i = 0; i < multiname.size(); i++) {
    ...
    s+=python3_lineno_wrapper(m_guard,funcname,4+m_lines_in_vars,4,
                             ", \"guard of action \\\"" + multiname[i].first +
                             "\\\"\")");

    /* actionXbody */
    s+=action_helper(m_body,"body",funcname,i,acnt);
    s+=python3_lineno_wrapper(m_body,funcname,4+m_lines_in_vars,4,
    ...
    s+=python3_lineno_wrapper(m_adapter,funcname,4+m_lines_in_vars,4,
...
std::string aalang_py3::stringify()
{
  s += indent(4,"\n    def adapter_init():\n") + "\n" +
