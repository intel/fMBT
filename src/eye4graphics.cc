                               ...
                               const int hayPixelSize)
{
    ...
    int samples = 16;
    int stepx = neex >= samples ? int(neex*scale)/samples : 1;
    if (stepx > 8) stepx = 8;
    for (int _x=0, _y=int(neey*scale)/2;
         _x < int(neex*scale);
         _x+=stepx) {
    ...
    samples = 8;
    stepx = neex >= samples ? int(neex*scale)/samples : 1;
    int stepy = neey >= samples ? int(neey*scale)/samples : 1;
