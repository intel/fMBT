...
def fmbtlog(msg, flush=True):
    try: open("/tmp/fmbt.fmbtlog", "a").write("%s\n" % (msg,))
    except: pass
...
def adapterlog(msg, flush=True):
    try:
        _adapterlogWriter(open(_g_adapterlogFilename, "a"),
...
def reportOutput(msg):
    try: open("/tmp/fmbt.reportOutput", "a").write("%s\n" % (msg,))
    except: pass
...
#
# 1. Temporarily copy-paste code after this comment to any Python 3
